...
class RationalBezierSurface(Surface):
    ...
    @staticmethod
    def fill_surface_from_three_boundaries(curve_1: BezierCurve3D or RationalBezierCurve3D,
                                           curve_2: BezierCurve3D or RationalBezierCurve3D,
                                           curve_3: BezierCurve3D or RationalBezierCurve3D) -> "RationalBezierSurface":
        """
        Creates a fill surface from three boundary curves by linearly interpolating ``curve_1`` and
        ``curve_2`` and displacing the edges created by the interpolation to form the ``curve_3`` boundary.

        Parameters
        ----------
        curve_1: BezierCurve3D or RationalBezierCurve3D
            Boundary curve 1
        curve_2: Bezier3D or RationalBezierCurve3D
            Boundary curve 2
        curve_3: Bezier3D or RationalBezierCurve3D
            Boundary curve 3
        ...
        """
        # Convert the boundary curves to rational Bézier curves if they are non-rational
        if isinstance(curve_1, BezierCurve3D):
            curve_1 = curve_1.to_rational_bezier_curve()
        if isinstance(curve_2, BezierCurve3D):
            curve_2 = curve_2.to_rational_bezier_curve()
        if isinstance(curve_3, BezierCurve3D):
            curve_3 = curve_3.to_rational_bezier_curve()

        # Ensure the boundary curve loop is closed
        cps_1 = curve_1.get_homogeneous_control_points()
        cps_2 = curve_2.get_homogeneous_control_points()
        cps_3 = curve_3.get_homogeneous_control_points()
        corners = np.array([
            cps_1[0],
            cps_1[-1],
            cps_2[0],
        ...
        # Elevate the curve degrees of the curve pairs so that they match internally
        if curve_1.degree != curve_2.degree:
            if curve_1.degree < curve_2.degree:
                while curve_1.degree < curve_2.degree:
                    curve_1 = curve_1.elevate_degree()
            else:
                while curve_2.degree < curve_1.degree:
                    curve_2 = curve_2.elevate_degree()
        ...
        # Linearly interpolate curves 1 and 2, length equal to number of control points
        # in the top/bottom direction
        arrays_to_stack = []
        for t in np.linspace(0.0, 1.0, curve_3.degree + 1):
            arrays_to_stack.append(cps_1 + t * (cps_2 - cps_1))
        Pw = np.array(arrays_to_stack)

        # Displace the control points associated with the curve 3
        for Pw_slice, curve_Pw in zip(Pw[1:-1], cps_3[1:-1]):
            displacement = curve_Pw - Pw_slice[0]
            for i in range(curve_1.degree):
                Pw_slice[i] += (curve_1.degree - i) / curve_1.degree * displacement
